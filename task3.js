...
fs.readFile("./transactions.csv", "utf-8", (err, data) => {
  ...
  for (let i = 1; i < splitData.length; i++) {
    ...
    if (users.get(user_id) !== undefined) {
      ...
      // add another transaction and update balance for an EXISTING transaction object
      if (transactions.has(username)) {
        const past_transactions = transactions.get(username).transactions,
          past_balance = transactions.get(username).balance;

        // update balance
        transactions.set(username, {
          ...transactions.get(username),
          balance: amount + past_balance,
        });
  ...
  console.log(transactions.get("Ola Botsford")); // test user (id: 80672672) to ensure that it returns the correct transactions
